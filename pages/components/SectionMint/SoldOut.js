export default function SoldOut(props){
    return(
        <section className="max-w-7xl mx-auto">
            <div className="bg-web3-band mx-auto max-w-7xl shadow-lg p-5 border border-slate-200 shadow-lg shadow-purple-500/30 flex flex-row justify-between justify-center items-center">
                <img src="./images/burgerNFT.png" className="w-[230px] mx-5"></img>
                <div className="w-2/3 flex flex-col items-center">
                    <h3 className="flex flex-col justify-center items-center text-center">
                        <span className="font-black text-5xl text-white tracking-wide">SOLD OUT !</span>
                        <span className="font-lighter text-xl tracking-wider mt-2">All Burger Cat are sold.</span>
                        <span className="bg-white px-3 p-1 rounded-xl font-black text-purple-500 text-4xl mt-4">{props.totalSupply+" / "+props.maxNFT}</span>
                    </h3>
                </div>
            </div>
        </section>
        )
}